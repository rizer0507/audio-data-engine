...
| | （ASR / 分拣见工序总览工序一表） | `*_asr_batch.yaml`、**默认** `prepare_dataset_v3` / `attach_asr_v3` / `audio_quality_sidecar` / `classify_dataset_v3`；legacy：`multi_asr_aggregate` / `asr_metric_pipeline` / `classify_dataset` |
