...
> `docs/04-改进需求/进行中/<名字>.md`（通常由 `docs/04-改进需求/需求文档模板.md` 用**自然语言**写成）。  
...
1. 打开并通读用户指定的 `docs/04-改进需求/进行中/...md`（未指定则让用户给出路径或直接粘贴需求）。
...
| 文档 | 角色 |
|------|------|
| `docs/04-改进需求/需求文档模板.md` | 用户用自然语言写需求 |
| **本文** | AI 翻译需求 + 工程落地 |
| `单条流水线执行命令.txt` | 生产命令台账（必须更新） |
| `docs/99-历史归档/流水线构建统一流程.md` | 历史文档；与本文冲突时以**本文 + 用户原话**为准 |
