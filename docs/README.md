...
最新 v3 修订与接入要求：[013-v3实现审计与代码修订交付](04-改进需求/进行中/013-v3实现审计与代码修订交付.md)。代码交付不等于生产验收，最终测试由使用者执行。

系统目前必须遵守的规则（Single Source of Truth）：

- [数据源登记规范](02-规范规则/数据源登记规范.md) — `resources/manifest.yaml` 字段与登记约定
- [数据挑选规则](02-规范规则/数据挑选规则.md) — 与 `configs/selection/zh_asr_v1.yaml` 一致的现行分拣规则（**工序一**③）
...
- [012-四家族双跑分拣与人工金标数据集闭环重构](04-改进需求/进行中/012-四家族双跑分拣与人工金标数据集闭环重构.md) — v3 目标方案：四家族双跑、Qwen 纠错、DNSMOS、人审与训练/双评测集；细分见 [012-细分需求索引](04-改进需求/进行中/012-细分需求索引.md)（**012-A～E 工程均已完成**；DNSMOS 权重、人工听审、伪标签抽检与业务门禁阈值校准待补；**未通过验收不得切换生产默认**）
...
- [005-评测流水线改进需求](04-改进需求/已完成/005-评测流水线改进需求.md) — 外源金标场景 1/2 正式路径
- [006-工序一清洗引擎拆分需求](04-改进需求/已完成/006-工序一清洗引擎拆分需求.md) — `classify_external_gold` → `classified_`
- [010-挑选规则重构](04-改进需求/已完成/010-挑选规则重构.md) — consensus 引擎 / medoid / 8 类分桶

## 设计决策

- [设计决策索引](05-设计决策/README.md) — 正式 ADR 待抽离；决策线索见该页
- [009-噪声评分流水线接入](04-改进需求/进行中/009-噪声评分流水线接入.md) — DNSMOS→`noise_risk`；**引擎不做 VAD**（输入已是上游切分段）
